		...
		$('#paging').change(function(){
			...
			$.ajax({
				...
				success:function(responsedata){ 
					...
					$('#zero_config_info').append("총 부서원 " + responsedata.totalempcount);
			...
			$.ajax(
					 {
						 ...
						 success:function(responsedata){ 
							 console.log(responsedata);
							$('#emptable').empty();
							$.each(responsedata.emplistByEmpno,function(index,obj){	
								$('#emptable').append(	
										"<tr><td>"+obj.empno+"</td>" +
										"<td><a href='EmpDetail.emp?empno="+obj.empno+"&cp=${cpage}&ps=${pagesize}'>" +
											obj.ename+"</a></td>" +
								...
								$('#totalempcount').val(responsedata.totalempcount);	
								//페이지 번호 처리
								page(responsedata.cpage);
						 }
			...
			$.ajax(
					 {
						 ...
						 success:function(responsedata){ 
							 console.log(responsedata);
							$('#emptable').empty();
							$.each(responsedata.emplistByDeptno,function(index,obj){	
								$('#emptable').append(	
										"<tr><td>"+obj.empno+"</td>" +
										"<td><a href='EmpDetail.emp?empno="+obj.empno+"&cp=${cpage}&ps=${pagesize}'>" +
											obj.ename+"</a></td>" +
						...
						<div id="zero_config_wrapper"
									...
									<table id="zero_config"
										...
										<thead>
											<tr role="row" >
												...
												<th class="sorting" tabindex="0" aria-controls="zero_config"
													rowspan="1" colspan="1"
													aria-label="사번: activate to sort column ascending">회원</th>
												<th class="sorting" tabindex="0" aria-controls="zero_config"
													rowspan="1" colspan="1"
													aria-label="이름: activate to sort column ascending">포인트</th>
												<th class="sorting" tabindex="0" aria-controls="zero_config"
													rowspan="1" colspan="1"
													aria-label="직급: activate to sort column ascending">출석상태</th>
											</tr>
										</thead>
										<tbody id="emptable">
										...
										<c:forEach var="user" items="${userList}">
												<tr>
													...
													<td>${user.upoint}</td>
													<td>
														<c:if test="${user.cstate eq 'Y'}">
															<input type="checkbox" name="check"  value="${user.userid }" checked>
														</c:if>
															<c:if test="${user.cstate eq 'N'}">
							...
							<div class="row">
								...
								<div class="col-sm-12 col-md-7">
									...
									<button type="submit" align="center">전송</button>
							</form>
							<form action="index.htm"><button type="submit">갱신</button></form>
