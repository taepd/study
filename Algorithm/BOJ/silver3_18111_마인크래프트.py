...
1 초 (추가 시간 없음)	1024 MB	4056	1333	1054	30.873%
문제
팀 레드시프트는 대회 준비를 하다가 지루해져서 샌드박스 게임인 ‘마인크래프트’를 켰다.
마인크래프트는 1 × 1 × 1(세로, 가로, 높이) 크기의 블록들로 이루어진 3차원 세계에서 자유롭게 땅을 파거나
집을 지을 수 있는 게임이다.

목재를 충분히 모은 lvalue는 집을 짓기로 하였다. 하지만 고르지 않은 땅에는 집을 지을 수 없기 때문에
...
1번 작업은 2초가 걸리며, 2번 작업은 1초가 걸린다. 밤에는 무서운 몬스터들이 나오기 때문에 최대한 빨리 땅 고르기
 작업을 마쳐야 한다. ‘땅 고르기’ 작업에 걸리는 최소 시간과 그 경우 땅의 높이를 출력하시오.

단, 집터 아래에 동굴 등 빈 공간은 존재하지 않으며, 집터 바깥에서 블록을 가져올 수 없다.
또한, 작업을 시작할 때 인벤토리에는 B개의 블록이 들어 있다. 땅의 높이는 256블록을 초과할 수 없으며,
음수가 될 수 없다.
...
arr = []
for _ in range(m):
    for e in input().rstrip().split():
        arr.append(int(e))

# 인벤토리를 포함한 높이의 상한 설정
t = (sum(arr)+b)//len(arr)

# 계산 효율화를 위해 같은 높이의 블록 그룹화
c_list = Counter(arr).items()

ans = [float('inf'), -1]

# 상한 높이부터 아래로 순회하면서 최소시간 계산(같은 시간일 경우 가장 높은 높이도 자동으로 해결)
